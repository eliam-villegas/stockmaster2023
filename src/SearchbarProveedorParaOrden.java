...
public class SearchbarProveedorParaOrden extends JPanel{

    public SearchbarProveedorParaOrden(DefaultTableModel modelo){
    ...
    protected void realizarBusqueda(String filtro,DefaultTableModel modelo) {
        ...
        var rows = dbc.BuscarProveedor(filtro);
        for(Object[] row : rows){
            modelo.addRow(row);
        }
        if(modelo.getRowCount() == 0){
            JOptionPane.showMessageDialog(null, "No se han encontrado proveedores", "Mensaje", JOptionPane.INFORMATION_MESSAGE);
        }
    }
}
